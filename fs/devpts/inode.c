...
static int devpts_permission(struct inode *inode, int mask)
{
	...
	if (inode->i_ino == PTMX_INO ||
	    vx_check((xid_t)inode->i_tag, VS_WATCH_P | VS_IDENT))
...
{
	/* devpts is xid tagged */
	return de->d_inode->i_ino == PTMX_INO ||
	       vx_check((xid_t)de->d_inode->i_tag, VS_WATCH_P | VS_IDENT);
}
