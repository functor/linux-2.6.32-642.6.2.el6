            ...
            struct nx_info *nxi)
{
	...
	int err = 0;
	if (rt->rt6i_prefsrc.plen && v6_addr_in_nx_info(nxi, &rt->rt6i_prefsrc.addr, -1))
			 ...
			 int prefix, int nowait, unsigned int flags)
{
	...
	if (iif) {
		...
		if (ip6_route_get_saddr(net, rt, dst, 0, &saddr_buf, (skb->sk ? skb->sk->sk_nx_info : NULL)) == 0)
			NLA_PUT(skb, RTA_PREFSRC, 16, &saddr_buf);
	}

	if (rt->rt6i_prefsrc.plen && v6_addr_in_nx_info((skb->sk ? skb->sk->sk_nx_info : NULL), &rt->rt6i_prefsrc.addr, -1)) {
		struct in6_addr saddr_buf;
